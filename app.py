...
@app.on_event("startup")
def preload_news():
    # Cloud Run 可用環境變數直接關掉這段（保命用）
    if os.getenv("DISABLE_STARTUP_PRELOAD", "0") == "1":
        print("[NEWS] startup preload disabled")
        return

    def _job():
        try:
            refresh_who_news(limit=20, target_lang="zh-TW")
            print("[NEWS] 伺服器啟動後已預抓 WHO 快訊")
        except Exception as e:
            print(f"[NEWS] 預抓 WHO 快訊失敗: {e}")

    # 關鍵：背景執行，不阻塞 uvicorn listen
